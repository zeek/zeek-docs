...
Then, return to the terminal running Zeek and use Ctrl+C to exit. The logs may
have more than just the two entries found before since Zeek will analyze all
traffic on that network device. Entries should still appear in :file:`conn.log`,
...
request. You can run it through Zeek with the data from the pcap:
...
ZeekControl is also used to manage a cluster of Zeek processes. This cluster
contains many processes which analyze traffic together. Clusters are normally
split among many machines. For this example, all nodes will be local.
