...
In such scenarios, it would be more natural for Zeek to connect out to the
remote API, rather than the remote API to the Zeek cluster.
...
for this purpose is `websocat <https://github.com/vi/websocat>`_.
...
As Zeek only acts as a WebSocket server, but at the same time the
depicted remote API should not have knowledge about an arbitrary
number of Zeek clusters, the proposal is to decouple the components
using a WebSocket bridge for every Zeek cluster.
...
is best realized with events once the connection between the remote API and the
