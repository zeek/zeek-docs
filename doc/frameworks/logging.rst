...
Policy hooks provide a more powerful implementation of the logging predicates
that the framework has always supported. Those older predicates are harder to
augment and more difficult to pass state into, and are now deprecated. They
will get removed in Zeek 4.1, so if you’re using predicate functions, make sure
to migrate them to hooks. For reference, here’s an example of the old
mechanism:
...
This will result in a new log file :file:`conn-http.log` that contains only the
log records from :file:`conn.log` that are analyzed as HTTP traffic.
